...
[full-stack-fastapi-template-nextjs/blob/11c8a512f57e937aa623776418fa4cfb1e9b4dc4/frontend/Dockerfile](https://github.com/nemanjam/full-stack-fastapi-template-nextjs/blob/11c8a512f57e937aa623776418fa4cfb1e9b4dc4/frontend/Dockerfile)
...
[full-stack-fastapi-template-nextjs/blob/11c8a512f57e937aa623776418fa4cfb1e9b4dc4/.github/workflows/build-push-frontend.yml](https://github.com/nemanjam/full-stack-fastapi-template-nextjs/blob/11c8a512f57e937aa623776418fa4cfb1e9b4dc4/.github/workflows/build-push-frontend.yml)
...
[full-stack-fastapi-template-nextjs/blob/11c8a512f57e937aa623776418fa4cfb1e9b4dc4/frontend/package.json](https://github.com/nemanjam/full-stack-fastapi-template-nextjs/blob/11c8a512f57e937aa623776418fa4cfb1e9b4dc4/frontend/package.json)
...
Variables that are same for every environment are ok to be left as `NEXT_PUBLIC_` and let to be inlined in the bundle. Also, they should be versioned in Git (their `.env.*` files). Since it's that way, the best way is to simply store them as TypeScript constants directly in the source anyway, because that's what they truly are - the shared constants.

## Build and deploy reusable Docker image

Build once - deploy everywhere. Single image and `.env` file, no redundancy.
...
In [Static page](#static-page) section I already said few notes about runtime variables and static websites. Indeed, you have 2 options for runtime variables: 

1. Convert the website from static to dynamically rendered SSR (render at request time). Note, that this is significant change, from that point your website will require Node.js runtime and this will greatly impact your deployment options, since you can no longer use static hosting.
...
2. String replace directly on bundle assets with `sed`, `envsubst`, etc. This is the right direction to go. There are more options, like Nginx `subs_filter` config option, but be careful with it, it runs on each request and will waste CPU.

Another option to think of is to use `./env.js` file instead of usual `.env`, then you can host it with Nginx and load it as a `<script src="./env.js" />` into the app. Then you can reference variables with `window.__RUNTIME_ENV__.MY_VAR`. 

Note, this won't work well for usages in pure HTML pages. For example Astro omits any client JavaScript by default and you would need to use additional inline `<script />` tag to update the HTML with e.g. `getElementById("my-id")?.textContent = window.__RUNTIME_ENV__.MY_VAR` which is less optimal than string replace method.
...
So, to conclude, the best way is to use shell script `sed` or `envsubst` and add it to the Nginx `Dockerfile` `ENTRYPOINT` or `docker-compose.yml` `command:`. Here is the link to the already mentioned practical tutorial again https://phase.dev/blog/nextjs-public-runtime-variables/.
...
// todo:
remove comment file path
fix list paragraphs indentation
remove git hash from link text
