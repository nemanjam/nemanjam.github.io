...
description: |
  Build and deploy a static website as a reusable Docker image and see how practical it is.
...
## Introduction
 ...
 This article will be a live "try and see" practical experiment. I will use this exact blog project, static Astro website and try to package it as an reusable Nginx Docker image that will require just a single `.env` file to run in any environment.

 I will use this tutorial as a starting point https://phase.dev/blog/nextjs-public-runtime-variables/. It describes the idea and the process, it uses Next.js and has shell replacement script that we can work with.

## Goal

Let's define our goal and the requirements at the beginning. We will use a pure, static website that consists just from assets without any server runtime code. This is important because hosting static website is simple, free and widely available. We want to have reusable builds, no environment specific data is bundled within the app code, but read from a single `.env` file.

Now it's time to identify which data is environment specific. In this particular website it's these 4 environment variables:
...
PREVIEW_MODE=true
```
Notice the format of these variables, (almost) three url's and one boolean value.

Now let's understand clearly what makes it challenging: Once compiled, static website becomes just a collection of static assets (`.html`, `.js`, `.css`, `.jpg`, etc. files) deployed to a Nginx web folder. It practically means we have no server runtime and we cannot run any code, which greatly limits our power and control. The only runtime we have is the one in browser, which is only useful to some extent, when it comes to loading environment specific data at runtime.

## Options

Before we start following the original tutorial and the practical implementation, let's once again reconsider possible alternatives that we have at our disposal. I already touched on this in my previous article about the runtime environment variables: https://nemanjamitic.com/blog/2025-12-13-nextjs-runtime-environment-variables#alternative-approaches but lets go through it once again, since this time the entire article is dedicated to the runtime variables in purely static websites. Here are the alternatives:

1. The first one is the original idea from the tutorial: we will use a shell script with `sed` command to string-replace all placeholder values inlined at build-time directly in the bundle. We will include this script in the Docker entrypoint so it can run at container start and insert environment specific values. This way we will accomplish an effect of start-time variables.

2. Nginx has certain features to inject environment variables in responses. For example `sub_filter` can perform string replace in each text response. Upside of this is that it's truly runtime variable that will reflect any change, but the mayor disadvantage is the performance overhead, especially under big traffic load.

3. Another method is to rely on JavaScript runtime in the browser and dynamically host and load `<script src="./env.js" />` in the html of your root layout. It an have a few variations, for example, you can make `env.template` file that holds placeholders for the replacement script called in entrypoint, or you could even inline `env.js` in the `nginx.config` itself:
...
```
    ...
    The common thing is that you need to run client JavaScript on a given page to access the runtime variables through `window` object, e.g. `window.__RUNTIME_ENV__.MY_VAR`. This is disadvantage on its on and has performance cost, for example Astro enforces __zero client-side JavaScript by default__ strategy exactly for performance reasons. Another deal-breaker that files that don't run client JavaScript can't access these variables,  e.g. some of those are `sitemap.xml` and `robots.txt` which are a very important SEO related file especially in static websites.

## Implementation

And finally the most important and the most interesting part, the practical implementation of the most promising alternative. We spent quite a lot of time on introduction and the alternatives overview.

You can review this exact feature code in the pull request on this link: https://github.com/nemanjam/nemanjam.github.io/pull/28


### Replacement script

Let's start with the shell replacement script, we will use the `replace-variables.sh` from the tutorial as the starting point. After some trial and error and few iterations this is how the final script looks like:
...
- It uses `#!/bin/sh` shell syntax because `bash` is not available by default in Nginx Alpine image and we want to keep the image size minimal.
- It uses `DIST_PATH` environment variable used as argument to pass the path to the bundle into the script. On the top of the script we validate and initialize all the hardcoded and passed arguments and exit early in case of invalid data is provided.
- We support and handle separately both required and optional variables with `REQUIRED_VARS` and `OPTIONAL_VARS`.
- This is the important part: we treat ordinary string variables and and "url shaped" string variables separately, assign them separate identifying prefixes `PREFIX="BAKED_"` and `URL_PREFIX="https://${PREFIX}"` and placeholders `PLACEHOLDER="${PREFIX}${VAR}"` and `URL_PLACEHOLDER="${URL_PREFIX}${VAR}"` accordingly. This is needed because baked build for Astro will fail if we pass invalid url for `site` option in the `astro.config.ts`.
- We keep using `sed` over `envsubst` because of more control over string replacement. Additionally, we escape special chars `&` and `|` for `sed` input.
- We log processed files and replaced variables for debugging and monitoring purposes.

### Nginx image entrypoint

Once we have working and tested replacement script it's time to include it in the Docker entrypoint so it can run on the container start and replace baked placeholders in the bundle with the actual environment variables from the current environment.

Fortunately, Nginx Alpine image already provides dedicated pre-start folder `/docker-entrypoint.d` intended to place the entrypoint scripts. We define the `ENV DIST_PATH=/usr/share/nginx/html` environment variable because `replace-variables.sh` expects it as an input argument. Additionally, we include `10-` prefix in the script file name because it defines the scripts execution order in the entrypoint. We want to run our script before any other.

Bellow is the complete `runner` stage of the [docker/Dockerfile](https://github.com/nemanjam/nemanjam.github.io/blob/feature/runtime-environment-variables/docker/Dockerfile):
...
And finally, we define the actual values for environment variables in the [docker-compose.yml](https://github.com/nemanjam/nemanjam.github.io/blob/feature/runtime-environment-variables/docker-compose.yml) for test:
...
With this we have all set up to run the container and set the start-time environment variables. The original tutorial mentions certain tradeoffs of this method, like the slower container startup, the risk of unintentional string replacements but those are tolerable for our use case. But let's actually see if there is more to it, in detail, in the next section.
