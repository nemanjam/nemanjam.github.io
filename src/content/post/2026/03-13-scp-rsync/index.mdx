...
description: |
  Comparing the performance of different tools used for copying artifacts in automated deployment workflows.
...
Many of you can probably guess the point of this article just by reading the title, but it's still useful to have a clear reminder backed by some real-world measurements. This will be a short, straight-to-the-point article.
...
## Key rsync flags for deployments
...
The workflow implements the same logic using the [Burnett01/rsync-deployments](https://github.com/Burnett01/rsync-deployments) action. Since this is not a live terminal session, `--info=stats` is sufficient for logging.
